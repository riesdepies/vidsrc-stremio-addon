...
async function fetchViaProxy(url, options) {
    ...
    try {
        const proxyRes = await p({
            ...
            parse: 'json', // phin parseert de JSON-respons automatisch
            timeout: 15000 // Voeg een timeout toe
        });
        
        // De body van de phin response bevat de geparste JSON
        ...
        return {
            ...
            text: () => Promise.resolve(data.body) // data.body bevat de HTML text van de proxy
        };
    } catch (error) {
...
async function searchDomain(domain, apiType, imdbId, season, episode, controller, visitedUrls) {
    // De 'controller' wordt niet direct gebruikt door phin's timeout, maar we houden de logica voor afbreken in stand.
    ...
    for (let step = 1; step <= MAX_REDIRECTS; step++) {
        ...
        try {
            // De timeout wordt nu afgehandeld in fetchViaProxy
