...
// Helper functie voor het creëren van een vertraging
function sleep(ms) {
    ...
    for (let step = 1; step <= MAX_REDIRECTS; step++) {
        ...
        try {
            ...
            if (nextIframeSrc) {
                ...
                // Wacht een willekeurige tijd voor de volgende iframe fetch
                const randomWait = Math.floor(Math.random() * 401) + 100;
    ...
    for (const domain of domainQueue) {
        ...
        try {
            ...
            if (result) {
             ...
             if (error.name !== 'AbortError') {
                console.error(`[ERROR] Fout bij doorzoeken van domein ${domain}:`, error.message);
            }
        }
        
        // Wacht een willekeurige tijd voor het proberen van het volgende domein
        const randomWait = Math.floor(Math.random() * 401) + 100;
        await sleep(randomWait);
    }
